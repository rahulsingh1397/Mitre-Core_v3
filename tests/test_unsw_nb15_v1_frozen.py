"""
Frozen baseline integrity test for UNSW-NB15 v1.0.
...
FROZEN_DIR = REPO_ROOT / "benchmark" / "results" / "frozen" / "unsw_nb15" / "v1.0"
MANIFEST_PATH = FROZEN_DIR / "manifest.json"

# Frozen metric targets (eval split, mean over seeds 42/43/44, attack_cat track)
FROZEN_V3_ARI_MEAN = 0.5639
FROZEN_V3_AMI_MEAN = 0.6455
TOLERANCE = 0.01
...
class TestFrozenArtifactsExist:
    ...
    def test_required_files_present(self):
        required = [
            ...
            "environment.txt",
            "unsw_nb15_10000_seed42.npy", "unsw_nb15_10000_seed42.json",
            "unsw_nb15_10000_seed142.npy", "unsw_nb15_10000_seed142.json",
        ]
...
class TestHashIntegrity:
    def _unsw_dataset_entry(self, manifest: dict) -> dict:
        for ds in manifest.get("datasets", []):
            if "unsw" in ds.get("name", "").lower():
                return ds
        pytest.skip("UNSW-NB15 not found in manifest datasets")

    def test_checkpoint_hash_matches(self, manifest):
        ds = self._unsw_dataset_entry(manifest)
...
class TestDevEvalDisjoint:
    def test_dev_eval_splits_are_disjoint(self):
        dev_path = REPO_ROOT / "benchmark" / "splits" / "unsw_nb15_10000_seed42.npy"
        eval_path = REPO_ROOT / "benchmark" / "splits" / "unsw_nb15_10000_seed142.npy"
...
class TestFrozenMetricsReproducible:
    ...
    Excluded from normal CI — run with: pytest -m slow tests/test_unsw_nb15_v1_frozen.py
    """

    def _run_v3_on_unsw(self) -> "pd.DataFrame":
        import tempfile
        import pandas as pd
