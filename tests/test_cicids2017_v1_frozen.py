"""
Frozen baseline integrity test for CICIDS2017 v1.0.
...
Run with: pytest -m slow tests/test_cicids2017_v1_frozen.py
"""
...
FROZEN_DIR = REPO_ROOT / "benchmark" / "results" / "frozen" / "cicids2017" / "v1.0"
MANIFEST_PATH = FROZEN_DIR / "manifest.json"

# Frozen metric targets (eval split, mean over seeds 42/43/44, alert_type track)
# NOTE: These values are set at Stage 6 freeze time.
# If V3 won Path A: these are the default config values.
# If V3 ran a sweep (Path B): these are the sweep-winner values.
FROZEN_V3_ARI_MEAN = 0.17713770992281697   # label_col track, mean over seeds 42/43/44 (deterministic)
FROZEN_V3_AMI_MEAN = 0.5698886804675259   # label_col track, mean over seeds 42/43/44 (deterministic)
TOLERANCE = 0.01
...
class TestFrozenArtifactsExist:
    ...
    def test_required_files_present(self):
        required = [
            ...
            "environment.txt",
            "cicids2017_10000_seed42.npy", "cicids2017_10000_seed42.json",
            "cicids2017_10000_seed142.npy", "cicids2017_10000_seed142.json",
        ]
...
class TestHashIntegrity:
    def _cicids2017_entry(self, manifest: dict) -> dict:
        for ds in manifest.get("datasets", []):
            if "cicids" in ds.get("name", "").lower() or "cic" in ds.get("name", "").lower():
                return ds
        pytest.skip("CICIDS2017 not found in manifest datasets")

    def test_checkpoint_hash_matches(self, manifest):
        ds = self._cicids2017_entry(manifest)
...
class TestDevEvalDisjoint:
    def test_dev_eval_splits_are_disjoint(self):
        dev_path = REPO_ROOT / "benchmark" / "splits" / "cicids2017_10000_seed42.npy"
        eval_path = REPO_ROOT / "benchmark" / "splits" / "cicids2017_10000_seed142.npy"
...
class TestFrozenMetricsReproducible:
    """Verify V3 ARI/AMI on CICIDS2017 reproduce the frozen v1.0 values."""

    def _run_v3_on_cicids2017(self) -> "pd.DataFrame":
        import tempfile
        import pandas as pd
    ...
    def test_v3_ari_reproduces(self):
        if FROZEN_V3_ARI_MEAN is None:
            pytest.skip("Frozen ARI target not set — fill FROZEN_V3_ARI_MEAN at freeze")
        v3 = self._run_v3_on_cicids2017()
        v3_ari = v3["ari"].mean()
    ...
    def test_v3_ami_reproduces(self):
        if FROZEN_V3_AMI_MEAN is None:
            pytest.skip("Frozen AMI target not set — fill FROZEN_V3_AMI_MEAN at freeze")
        v3 = self._run_v3_on_cicids2017()
        v3_ami = v3["ami"].mean()
