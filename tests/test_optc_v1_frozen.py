"""
Frozen baseline integrity test for DARPA OpTC v1.0.
...
Run with: pytest -m slow tests/test_optc_v1_frozen.py
"""
...
FROZEN_DIR = REPO_ROOT / "benchmark" / "results" / "frozen" / "optc" / "v1.0"
MANIFEST_PATH = FROZEN_DIR / "manifest.json"

# Frozen metric targets (eval split, mean over seeds 42/43/44, campaign_id track)
# Headline metric for OpTC is binary_ARI, not standard ARI.
FROZEN_V3_BINARY_ARI_MEAN = 0.998647   # campaign_id track, mean over seeds 42/43/44 (deterministic)
FROZEN_V3_ARI_MEAN = 0.05863           # standard ARI (structurally low on 2-class dataset)
FROZEN_V3_AMI_MEAN = 0.202864          # AMI on campaign_id track
TOLERANCE = 0.01
...
class TestFrozenArtifactsExist:
    ...
    def test_required_files_present(self):
        required = [
            ...
            "environment.txt",
            "optc_10000_seed42.npy", "optc_10000_seed42.json",
            "optc_10000_seed142.npy", "optc_10000_seed142.json",
        ]
...
class TestHashIntegrity:
    def _optc_entry(self, manifest: dict) -> dict:
        for ds in manifest.get("datasets", []):
            if "optc" in ds.get("name", "").lower():
                return ds
        pytest.skip("DARPA-OpTC not found in manifest datasets")

    def test_checkpoint_hash_matches(self, manifest):
        ds = self._optc_entry(manifest)
...
class TestDevEvalDisjoint:
    def test_dev_eval_splits_are_disjoint(self):
        dev_path = REPO_ROOT / "benchmark" / "splits" / "optc_10000_seed42.npy"
        eval_path = REPO_ROOT / "benchmark" / "splits" / "optc_10000_seed142.npy"
...
class TestFrozenMetricsReproducible:
    """Verify V3 metrics on OpTC reproduce the frozen v1.0 values."""

    def _run_v3_on_optc(self) -> "pd.DataFrame":
        import tempfile
        import pandas as pd
        ...
        return results[
            ...
            (results["dataset"] == "DARPA-OpTC") &
            (results["label_track"] == "label_col")
        ].copy()

    def test_v3_binary_ari_reproduces(self):
        if FROZEN_V3_BINARY_ARI_MEAN is None:
            pytest.skip("Frozen binary_ARI target not set")
        v3 = self._run_v3_on_optc()
        v3_binary_ari = v3["binary_ari"].mean()
        assert abs(v3_binary_ari - FROZEN_V3_BINARY_ARI_MEAN) <= TOLERANCE, (
            f"V3 binary_ARI {v3_binary_ari:.4f} deviates from frozen {FROZEN_V3_BINARY_ARI_MEAN:.4f}"
        )

    def test_v3_ari_reproduces(self):
        if FROZEN_V3_ARI_MEAN is None:
            pytest.skip("Frozen ARI target not set")
        v3 = self._run_v3_on_optc()
        v3_ari = v3["ari"].mean()
    ...
    def test_v3_ami_reproduces(self):
        if FROZEN_V3_AMI_MEAN is None:
            pytest.skip("Frozen AMI target not set")
        v3 = self._run_v3_on_optc()
        v3_ami = v3["ami"].mean()
