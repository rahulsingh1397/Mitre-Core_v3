"""
Frozen baseline integrity test for TON_IoT v1.0.
...
Run with: pytest -m slow tests/test_ton_iot_v1_frozen.py
"""
...
FROZEN_DIR = REPO_ROOT / "benchmark" / "results" / "frozen" / "ton_iot" / "v1.0"
MANIFEST_PATH = FROZEN_DIR / "manifest.json"

# Frozen metric targets (eval split, mean over seeds 42/43/44, alert_type track)
# NOTE: V3 does NOT win on TON_IoT — K-Means (raw) ARI=0.622 is the best.
# These targets are for V3 reproducibility, not for claiming superiority.
FROZEN_V3_ARI_MEAN = 0.4228
FROZEN_V3_AMI_MEAN = 0.7054
TOLERANCE = 0.01
...
class TestFrozenMetricsReproducible:
    """
    Note: V3 does NOT win on TON_IoT (K-Means raw is best).
    These tests verify V3 reproducibility, not superiority.
    """
